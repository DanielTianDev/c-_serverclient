...
int __cdecl main(int argc, char **argv)
{
	...
	iResult = getaddrinfo("localhost", DEFAULT_PORT, &hints, &result); //127.0.0.1
	if (iResult != 0) {
	...
	while (strcmp(sendBuffer, "bye") != 0) {
		...
		cout << "Enter something to send ('bye' to quit):\n";
		//cin >> sendBuffer;
		cin.getline(sendBuffer, DEFAULT_BUFLEN);
...
int __cdecl main(int argc, char **argv)
{
	...
	while (sendBuffer[0] != '&') {
		...
		cout << "Enter something to send (& to quit):\n";
		cin >> sendBuffer;
		iResult = send(ConnectSocket, sendBuffer, (int)strlen(sendBuffer), 0);
